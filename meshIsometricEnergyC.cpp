...
void alpha_beta_from_fz2gz2(double fz2, double gz2, double* alphas, double* betas, int energy_type, double s=1.)
{
    ...
    case ISO_ENERGY_SARAP: {
        ...
        if (betas) {
            ...
            betas[1] = (gz2>1e-80) ?(-(1 - sig2) / pow3(sig2) - sig1 + 1 + gz2 * t ) / pow3(gz2) / 2 : 10;
            betas[2] = (gz2>1e-80) ?(2-t) / fz2 / gz2 / 2 : -6;
        }
        break;
    }
    case ISO_ENERGY_HOOK: {
...
void mulMatrixMKM(const Matrix4d &K, cvec &R, cvec &I, mat& A2)
{
    ...
    B << k[4] - 2 * k[6] + k[9], k[3] - k[1] + k[5] - k[8], 0, k[0] - 2 * k[2] + k[7];
    A2.block<3, 3>(3, 3) = B(0,0)*RtR + B(1,1)*ItI + B(0,1)*(RtI + RtI.transpose());

    B << k[3] - k[1] + k[8] - k[5], k[0] - k[7], k[9] - k[4], k[1] + k[3] - k[5] - k[8];
...
double evalIsometricEnergy(const double *fzr, const double *fzi, const double *gzr, const double *gzi, cmat &Dr, cmat &Di, const double *area,
    int energy_type, double s, double *grad, double *hessian, int projMethod, int &NE)
{
    const int n = (int)Dr.cols();
    double en = 0;
    int NEA = 0;
    #pragma omp parallel for shared(fzr, fzi, gzr, gzi, Dr, Di, area, grad, hessian) reduction(+: en, NEA)
    for (int i = 0; i < n; i++) {
        ...
        if (hessian) {
            ...
            if (projMethod == HESS_PROJ_CM) {
                if (energy_type == ISO_ENERGY_ARAP || energy_type == ISO_ENERGY_EXP_SD 
                  ...
                  double mu = s; double kappa = 1-s;
                  double xpy = fz2+gz2;double xmy = fz2-gz2;
...
#pragma omp parallel
    ...
    t = steady_clock::now();
    int NE = 0;
    plhs[0] = mxCreateDoubleScalar( evalIsometricEnergy(fzr, fzi, gzr, gzi, Dr, Di, Area, energy_type, energy_param, grad, hessian, hessProj,NE) );
    if (nlhs > 3) plhs[3] = mxCreateDoubleScalar(NE);
    if (verbose) {
        char str[100];
